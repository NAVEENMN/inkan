...
class TestStrictNonNegativity:
    ...
    def test_derivative_consistency(self):
        ...
        # Use a single input feature so shapes are simple
        x_vals = torch.linspace(-0.8, 0.8, 100)
        eps = 1e-4

        # Finite difference per basis: [100, n_bases]
        b_plus = bspline_basis_eager(
            (x_vals + eps).unsqueeze(0), grid_starts, inv_h).squeeze(0)
        b_minus = bspline_basis_eager(
            (x_vals - eps).unsqueeze(0), grid_starts, inv_h).squeeze(0)
        d1_fd = (b_plus - b_minus) / (2 * eps)

        # Autograd per basis: iterate over each basis function
        for bi in range(n_bases):
            x_ad = x_vals.unsqueeze(0).clone().requires_grad_(True)
            b = bspline_basis_eager(x_ad, grid_starts, inv_h)
            b[:, :, bi].sum().backward()
            d1_ad_bi = x_ad.grad.squeeze(0)  # [100]
